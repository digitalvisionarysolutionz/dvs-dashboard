...
function buildCalendarSummary(events = []) {
  ...
  const projectDeadlines = events.filter(
    (event) => event.category === "deadlines"
  );
  const leadFollowUps = events.filter(
    (event) => event.category === "follow_ups"
  );
  const intakeEvents = events.filter(
    (event) => event.category === "intake_submissions"
  );
